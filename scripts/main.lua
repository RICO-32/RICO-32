local player = require("player") -- standard Lua

function start()
    rico:log("Start function")
end

function button(x, y, w, h, label)
    rico:rectfill(x, y, w, h, "CYAN")
    rico:rect(x, y, w, h, "MAGENTA")
    rico:print_scr_mini(x+4, y+4, "PURPLE", label)
    local mouse = rico:mouse()
    if mouse.just_pressed then
        if mouse.x >= x and mouse.x <= x + y and mouse.y >= y and mouse.y <= y+h then
            rico:log("Button just pressed")
        end
    end
    if mouse.pressed then
        if mouse.x >= x and mouse.x <= x + y and mouse.y >= y and mouse.y <= y+h then
            rico:log("Button pressed")
        end
    end
end

math.randomseed(os.time())

function update(dt)
    player:upd()
    rico:clear("BLACK")
    rico:print_scr(1, 50, "GREEN", "WOWAZA")
    rico:draw(1, 45, "correct.png")
    button(60, 50, 40, 10, "BUTTON!")
    rico:circle(90, 90, 10, "PURPLE")
    if rico:key_just_pressed("Enter") then
        rico:log("Just pressed enter")
    end
    
    if player.x % 10 == 0 then
        rico:log("Frame rate: " .. tostring(math.floor(1000/dt)))

        local x = math.random(0, 127)
        local y = math.random(0, 127)
        local cols = {"GREEN", "CYAN", "BLUE", "RED", "TEAL"}
        local col = cols[math.random(1, #cols)]
        if rico:key_pressed("Enter") then
            rico:log("Pressed enter")
        end
        rico:set_pix(x, y, col)
        rico:log("Pixel " .. tostring(x) .. " " .. tostring(y) .. " is " .. rico:get_pix(x,y))
    end
    if player.x % 20 == 0 then
        rico:clear("GREEN")
    end
    if player.x % 100 == 0 then
        local new_frame_rate = math.floor(player.x / 10)
        rico:log("Changing framerate to " .. tostring(new_frame_rate))
        rico:set_frame_rate(new_frame_rate)
    end
end

